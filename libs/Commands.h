...
class ICommand {
 public:
  virtual int execute(Field& field, Galaxy& galaxy,
                      std::vector<std::string> args) = 0;
};

class Exit : public ICommand {
 public:
  int execute(Field& field, Galaxy& galaxy,
...
class ExitCreator : public ICommandCreator {
  ICommand* create() override { return new Exit; }
};

class HelpCreator : public ICommandCreator {
  ICommand* create() override { return new Help; }
};

class TickCreator : public ICommandCreator {
  ICommand* create() override { return new Tick; }
};

class PlayCreator : public ICommandCreator {
  ICommand* create() override { return new Play; }
};

class DumpCreator : public ICommandCreator {
  ICommand* create() override { return new Dump; }
};
