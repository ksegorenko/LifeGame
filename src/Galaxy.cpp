...
std::vector<bool>& Galaxy::GetField() { return this->Field; }

int Galaxy::GetSize() { return this->Size; }

std::string Galaxy::GetName() { return this->Name; }

Rules& Galaxy::GetRules() { return *(this->Rule); }
...
int GetGalaxySize(std::string& str, std::ifstream& fin) {
  ...
  if (std::regex_match(str.c_str(), Result, Regular))
    ...
    std::cout << "WARNING. No input galaxy size. Game will start with default "
                 "galaxy size = 10."
              << std::endl;
    Size = 10;
  }
...
Rules* GetGalaxyRules(std::string& str, std::ifstream& fin) {
  ...
  if (std::regex_match(str.c_str(), Result, Regular)) {
    ...
    rules->SetBirthRules(s);
    rules->SetSurviveRules(Result[7]);
    std::getline(fin, str);
  } else {
    std::cout
        << "WARNING. No input galaxy rules. The game will start with B2/S23\n"
        << std::endl;
...
std::vector<bool> GetFieldVector(std::string& str, std::ifstream& fin,
                                 int Size) {
  ...
  while (std::getline(fin, str)) {
    if (std::regex_match(str.c_str(), Result, Regular)) {
      ...
      std::cout << "WARNING. Cell (" << Result[1] << "," << Result[3]
                << ") is out of galaxy range.\n";
    if (IsFieldEmpty) {
...
void operator>>(std::ifstream& fin, Galaxy& Galaxy) {
  ...
  Galaxy.Rule = GetGalaxyRules(str, fin);
