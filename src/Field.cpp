...
int Field::GetSize() { return this->Size; }
...
bool Field::GetCell(int x, int y) const { return CurrentField[x * Size + y]; }
...
int GetUpper(const std::vector<bool>& Field, int CurrentPosition, int Height,
             int Width) {
...
int GetUpperRight(const std::vector<bool>& Field, int CurrentPosition,
...
int GetRight(const std::vector<bool>& Field, int CurrentPosition, int Height,
             int Width) {
...
int GetBottomRight(const std::vector<bool>& Field, int CurrentPosition,
                   int Height, int Width) {
  if (((CurrentPosition + 1) % Width == 0) ||
      (CurrentPosition >= (Height * Width - Width))) {
    return 0;
  } else {
    return Field[CurrentPosition + Width + 1] ? 1 : 0;
  }
...
int GetBottom(const std::vector<bool>& Field, int CurrentPosition, int Height,
              int Width) {
...
int GetBottomLeft(const std::vector<bool>& Field, int CurrentPosition,
                  int Height, int Width) {
...
int GetLeft(const std::vector<bool>& Field, int CurrentPosition, int Height,
            int Width) {
  if (CurrentPosition % Width == 0) {
    return Field[CurrentPosition + Width - 1] ? 1 : 0;
  } else {
...
int GetUpperLeft(const std::vector<bool>& Field, int CurrentPosition,
                 int Height, int Width) {
...
int GetNeighbors(const std::vector<bool>& Field, int CurrentPosition,
                 int Height, int Width) {
...
void Field::CreateNext(Galaxy& Galaxy) {
  ...
  for (int i = 0; i < Size; ++i) {
    for (int j = 0; j < Size; ++j) {
      ...
      bool IsStayAlive =
          IsSurvive(Neighbors, Galaxy.GetRules().GetSurviveRules()) &&
          IsAlive(CurrentField[CurrentPosition]);
      bool IsBecameAlive = !IsAlive(CurrentField[CurrentPosition]) &&
                           IsBorn(Neighbors, Galaxy.GetRules().GetBirthRules());
